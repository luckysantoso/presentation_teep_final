...
\beat
\say{The seed is from a bot in the \emphw{middle of the warehouse — away from the pick and replenishment stations} — so we are looking at genuine corridor dynamics. Left: the seed observation reconstructed by the VAE. Right: \textbf{five 7-step dream rollouts, each one pinned to a different Gaussian mode} — k = 1 through 5. Below each, the mixture weight $\pi_k$ with a small bar.}

\stage{point along the panels}

\say{For this seed, the weights are \textbf{0.05, 0.23, 0.35, 0.29, and 0.08} — four components carry meaningful probability mass.}
\beat
\say{After seven dream steps, the modes diverge into \emphw{visibly different scenes}: k = 1 drifts forward; k = 2 stays under a pod; \textbf{k = 3 collapses into a different aisle perspective}; k = 4 keeps the overhead pod; k = 5 is a slight variant.}
\beat
\say{\emphw{One seed} $\rightarrow$ five imagined trajectories, each weighted by $\pi_k$. This is what genuine multimodal dynamics looks like — the model is not just adding noise to a single mean, it is predicting \textbf{structurally different futures}. CMA-ES averages over these in dream rollouts to score policies — \textbf{no real environment touched}.}

\begin{stagebox}
The five-futures figure is the focal point — use it. ``Five possible futures, weighted'' — quotable, pause around it. The 7-step horizon is what makes the modal divergence visible — at 1 step the warehouse barely changes. $\rightarrow$ Slide 10.
\end{stagebox}
\end{slide}
